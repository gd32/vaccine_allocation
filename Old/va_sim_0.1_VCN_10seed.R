...
seed_number = 10 # 1 new infection per round
e_period = 3
i_period = 3
r_period = 300
period = 300
...
for (h in 1:iters) {
  ...
  contact_matrix = as.matrix(xdata0 %*% ifelse(ndata1$symptomatic == 0 & ndata1$state %in% c(1, 3, 4, 5, 6), 1,0))
  ...
  for (m in 1:period){ 
    # if(tail(ci_sym, n = 1) <0.1){
      ...
      print(paste0("Round ", m, " for iteration ",h, " is done at ", Sys.time()))
    }   
...
write_csv(sim_result_total, "sim_result_post_vac_VCN_10seed_0205.csv")
write_csv(sim_result_sym, "sim_result_post_vac_sym_only_VCN_10seed_0205.csv")
