...
for(h in 1:iters){
  set.seed(h)
  g0 = sample_smallworld(dim = 1, size = n, nei = 7, p = p) 
...
r_period = 300
period = 300
available_vaccines = 50 #500 for 10000 people - 200? 500?
iters = 100
...
for (h in 1:iters) {
  ...
  for(k in 1:25){
    top10 = sort(betweenness(g1), decreasing = TRUE)[1:2]
...
write_csv(sim_result_total_pv, "sim_result_post_vac_1cperweek_block2_0205.csv")
write_csv(sim_result_sym_pv, "sim_result_post_vac_1cperweek_block2_sym_only_VHC_0205.csv")
