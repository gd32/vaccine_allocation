...
for(h in 1:iters){
  ...
  diag(xdata) = 0
  writeMM(obj = xdata, file = paste0("xdata_h", h, "_0131.mtx"))
...
# plot(g0, layout = layout.kamada.kawai, vertex.size = 5, vertex.label = NA)
  ...
  ndata$contacts = NA
  head(ndata)
  write_csv(ndata, file = paste0("ndata_R0_h", h, "_0131.csv"))
...
for (h in 1:10){
    ...
    ndata1 = read_csv(paste0("ndata_R0_h", h, "_0131.csv"))
    xdata0 = readMM(paste0("xdata_h", h, "_0131.mtx"))
      ...
      #NOTE: This is fine because individuals would have at most 1 infection in lifetime here in the simulations.
        ...
        for (m in 1:period) {
          ...
          if(tail(cis, n = 1) <= 0.3){
            ...
            write_csv(ndata1, file = paste0("ndata1_h_",h,"_ci_limit_reached_0201.csv"))
            break
...
for (h in 1:1){
  #Simulate infection using SEIR model
  ndata1 = read_csv(paste0("ndata1_h_",h,"_ci_limit_reached_rd_",m,"_0201.csv")))
  xdata0 = readMM(paste0("xdata_h", h, "_0131.mtx"))
  ...
  sim_result = as.data.frame(rbind(sim_result, result_vector))
