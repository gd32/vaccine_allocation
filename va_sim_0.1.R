...
for(h in 1:iters){
  ...
  diag(xdata) = 0
  writeMM(obj = xdata, file = paste0("xdata_h", h, "_0202.mtx"))
...
# plot(g0, layout = layout.kamada.kawai, vertex.size = 5, vertex.label = NA)
  ...
  ndata$contacts = NA
  head(ndata)
  write_csv(ndata, file = paste0("ndata_R0_h", h, "_0202.csv"))
...
write_csv(net_result, file = paste0("net_results_0131.csv"))
...
sim_result = c(0, rep(0, period), rep(0, period), rep(0, period))
names(sim_result) = c("h",
                         paste0("n_",1:period),#n_x = x-th round new exposure (infection)
                         paste0("p_",1:period),#p_x = x-th round prevalence
                         paste0("c_",1:period))#c_x = x-th round cumulative incidence,
...
for (h in 1:10){
    ...
    ndata1 = read_csv(paste0("ndata_R0_h", h, "_0202.csv"))
    xdata0 = readMM(paste0("xdata_h", h, "_0202.mtx"))
    ...
    ndata1$infection = sample(c(rep(1, new_infections), rep(0, n - new_infections)), size = n)
    ...
    prevs = 0 #prevalence (proportion of the latent/infectious people)
      ...
      #NOTE: This is fine because individuals would have at most 1 infection in lifetime here in the simulations.
        ...
        for (m in 1:period) {
          ...
          #A0 - Add 1 case per round
          ndata1[ndata1$infection == 0, "infection"] = sample(c(rep(1, new_infections), rep(0, n - m - new_infections)), size = n - m)
          ...
          if(tail(cis, n = 1) <= 0.1){
            ...
            cis = c(cis, 999)
            prevs = c(prevs, 999)
            print(paste0("Simulation stopped when reaching CI = 0.3 at round ", m))
          ...
          # if(tail(cis, n =1) >= 0.3){
          #   print(paste0("Simulation stopped when reaching CI of 30% at round ", m))
          #   break
          #   }
...
#Save results
write_csv(sim_result, file = paste0("sim_result_pre_vac_",as.numeric(format(Sys.Date(), "%m%d")),".csv"))
...
for (h in 1:1){
  ...
  # Continue the simulation on the day after we stopped before
  
  last_day = as.numeric(str_extract(file_to_use, pattern = regex("\\d{1,2}\\.")))

  for (day in last_day:period) {
    ...
    #Setting 1 - Vaccinating Close Contacts of Previously Diagnosed Individuals (VCD)
    ...
    #NOTE: The tag for the state change is "state_end", which turns to be the same as the present "round"
      ...
      as.matrix(xdata0 %*% ifelse(ndata1$state %in% c(4,5),1,0))
      
      ndata1$contacts = as.matrix(xdata0 %*% ifelse(ndata1$state %in% c(4,5),1,0)) #contact with infected (state== 4 or 5)
      
      ndata1[ndata1$symptomatic]
      ...
      if(tail(cis, n = 1) <= 0.3){
        ...
        #Save ndata at this point to implement vaccination
        write_csv(ndata1, file = paste0("ndata1_h_",h,"_ci_limit_reached_rd_",m,"_0201.csv"))
        break
    ...
    #Setting 2 - Vaccinating Close Contacts of Previously Undiagnosed Individuals Only (VCN)
      
      
    #Setting 3 - Vaccinating Highest Centrality Individuals (VHC)  
        
    
  ### need to figure out how to have diff. length rows for each iteration since CI = 0.30 reached at different times.
  
  new_results = c(h,new_exp,prevs,cis) #add h for random seed
  
  names(new_results) = c("h",
                           paste0("n_",1:length(new_exp)),#n_x = x-th round new exposure (infection)
                           paste0("p_",1:length(cis)),#p_x = x-th round prevalence
                           paste0("c_",1:length(prevs)))#c_x = x-th round cumulative incidence,
  
  sim_result = as.data.frame(rbind(result_vector, new_results))
