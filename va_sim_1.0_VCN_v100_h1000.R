...
for (h in 1:iters) {
  ...
  #Strategy 2 - Vaccinating close contacts of previously undiagnosed individuals only (VCN) strategy 
  contact_matrix = as.matrix(xdata0 %*% ifelse(ndata1$symptomatic == 0 & ndata1$state %in% c(0, 1, 4, 5, 6), 1,0))
  ...
  for (m in halt_round+1:period){
    ...
    #B1. For possible state change from S(0) to E(1)
    ndata1$contacts = as.matrix(xdata0 %*% ifelse(ndata1$state %in% c(4,5),1,0)) #contact with I (state== 4 or 5)
    ndata1 %>% count(state %in% c(1,3,4,5,6))
    #NOTE: people with S(0) gets infected from individuals who are not self-isolated = I(4) or I(5)
    #NOTE: sum them all (with weights) are the number of contacts in the present round
    ndata1$new_e = as.numeric(lapply(1-(1-infection_rate)^(ndata1$contacts), sample1)) #possible infections 
  ...
  names(result_vector_total_pv) = c("h",
                                    paste0("n_",halt_round+1:period),#n_x = x-th round new exposure (infection)
                                    paste0("p_",halt_round+1:period),#p_x = x-th round prevalence
                                    paste0("c_",halt_round+1:period))#c_x = x-th round cumulative incidence,
  
  names(result_vector_sym_pv) = c("h", 
                                  paste0("n_",halt_round+1:period),#n_x = x-th round new exposure (infection)
                                  paste0("psym_",halt_round+1:period),#p_x = x-th round prevalence
                                  paste0("csym_",halt_round+1:period))#c_x = x-th round cumulative incidence,
...
write_csv(sim_result_total_pv, paste0("sim_result_VCN_v",available_vaccines,"_iters",iters"_0211.csv"))
# write_csv(sim_result_sym_pv, "sim_result_sym_VCN_v100_0210.csv")
