...
r_period = 300
period = 300
available_vaccines = 500 #500 for 10000 people
iters = 100
...
for (h in 1:iters) {
  ...
  names(result_vector_sym_pv) = c("h", 
                                  paste0("n_",halt_round:period),#n_x = x-th round new exposure (infection)
                                  paste0("psym_",halt_round:period),#p_x = x-th round prevalence
                                  paste0("csym_",halt_round:period))#c_x = x-th round cumulative incidence,
...
write_csv(sim_result_total_pv, "sim_result_VCN_v500_0210.csv")
write_csv(sim_result_sym_pv, "sim_result_sym_VCN_v500_0210.csv")
